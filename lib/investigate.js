...
export function analyzeChunk(text = '') {
  ...
  const chemRe = /\b(?:CH4|N2O|CO2|CO2e|GWP20|GWP100|GWP|EPSG:?\d{3,5}|GeoTIFF|methane|manure|fermentation|emissions?)\b/gi;
  const metricRe = /\b(?:uncertainty|upper bound|lower bound|capacity factor|activity|pixel|pasture|country total|global warming potential)\b/gi;
  ...
  const countryLike = new Set(['United States','United Kingdom','Saudi Arabia','South Africa','New Zealand','Costa Rica','El Salvador','Sri Lanka']);
  for (const m of value.matchAll(personRe)) {
    const v = m[1];
    if (countryLike.has(v)) {
      entities.push({ type: 'location', value: v, normalized: v.toLowerCase(), confidence: 0.75 });
      continue;
    }
    entities.push({ type: 'person', value: v, normalized: v.toLowerCase(), confidence: 0.55 });
  }
  ...
  for (const m of value.matchAll(chemRe)) {
    const v = m[0];
    const type = /epsg|geotiff/i.test(v) ? 'geo_standard' : (/gwp|ch4|n2o|co2/i.test(v) ? 'compound_or_metric' : 'domain_term');
    entities.push({ type, value: v, normalized: String(v).toLowerCase(), confidence: 0.78 });
  }
  for (const m of value.matchAll(metricRe)) {
    const v = m[0];
    entities.push({ type: 'domain_metric', value: v, normalized: String(v).toLowerCase(), confidence: 0.72 });
  }
  ...
  const ratioRe = /\b\d+(?:\.\d+)?\s*(?:%|kg|t|tonnes?|tons?)\b/gi;
  for (const m of value.matchAll(ratioRe)) {
    events.push({ type: 'measurement', rawAmount: m[0], dates, confidence: 0.66 });
  }
