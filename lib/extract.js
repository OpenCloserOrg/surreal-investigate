...
const SUPPORTED = new Set(['txt', 'md', 'csv', 'json', 'eml', 'sql', 'pdf', 'docx', 'xlsx', 'xls', 'doc', 'epub']);
...
export async function extractTextFromFile(absPath, filename = '') {
  ...
  try {
    if (['txt', 'md', 'csv', 'eml', 'sql'].includes(ext)) {
    ...
    if (ext === 'pdf') {
      ...
      const text = sanitizeRawText(parsed?.text || '');
      if (text) return { supported: true, text, method: 'pdf-parse' };
      return { supported: true, text: extractRawFallback(buffer), method: 'pdf-fallback' };
    }
